...
use rockserver_client::proto::{
    put_batch_request, Kv, PutBatchInitialRequest, PutBatchRequest, PutRequest, RequestContext,
    WorkloadProfile,
};
...
fn unary_request_retains_mandatory_workload_context() {
    let context = RequestContext {
        ...
        timeout_nanos: RequestContext::NO_TIMEOUT,
    };
...
fn typed_context_constructor_rejects_protected_profiles() {
    for profile in [
        ...
        assert!(std::panic::catch_unwind(|| RequestContext::for_profile(profile, None)).is_err());
    }
}
...
fn raw_sst_tokens_accept_only_canonical_opaque_table_names() {
    ...
    for invalid in [
        ...
        assert!(
            RawSstToken::new(invalid.to_owned()).is_err(),
            "accepted {invalid}"
        );
    }
}
...
fn typed_context_constructor_uses_explicit_selectable_wire_values() {
    for (profile, expected) in [
        ...
        let timeout = (profile == WorkloadProfile::Latency).then(|| Duration::from_nanos(1));
        let context = RequestContext::for_profile(profile, timeout);
        assert_eq!(context.profile, expected);
        assert_eq!(
            context.workload_contract_version,
...
#[test]
fn finite_timeout_saturates_without_becoming_no_timeout() {
    let context = RequestContext::batch_with_timeout(Duration::MAX);
    assert_eq!(context.timeout_nanos, i64::MAX - 1);
}
