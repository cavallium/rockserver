...
`api.merge(key, delta)` through a context-bound API view.
...
*   **Simplified Architecture**:
    *   **Write Path**: `client -> api.merge()` (Fast)
    *   **Read Path**: `indexer -> api.cdcPoll(resolved=true) -> index.put()` (Simple)
...
. **Setup**:
    ...
    *   Create CDC Subscription: `api.cdcCreate("index-sub", startSeq=0, columns=[col1], emitLatestValues=true)`.

. **Write**:
    *   Client calls `api.merge(key, delta)`.
    *   RocksDB appends to WAL and MemTable.

. **Index**:
    *   Indexer calls `api.cdcPoll("index-sub")`.
    *   Rockserver scans WAL.
    *   Finds `MERGE` on `key`.
    *   Rockserver performs an internal read of `key` -> returns "OldValue + Delta".
...
long startSeq = api.cdcCreate(subId, null, List.of(columnId), emitLatestValues);

// Poll Loop
while (running) {
    List<CDCEvent> events = api.cdcPoll(subId, null, 100).toList();
    ...
    // Commit progress
   	if (!events.isEmpty()) {
		api.cdcCommit(subId, events.get(events.size() - 1).seq());
	}
}
----
